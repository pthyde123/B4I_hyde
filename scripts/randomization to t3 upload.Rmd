...
randomization <-Final_pea_oat_experiment_design <- read_excel("data/Final_pea_oat_experiment_design.xlsx") %>% 
  rename(accession = Oat) %>% 
  rename(intercrop_accession_name = Pea) %>% 
  rename(location = Location) %>% 
  rename(plotNo = PlotNumber) %>% 
  select(plotNo,location,accession,intercrop_accession_name)
...
# we have no layouts at this point, they will be added in later
...
trial_name = "B4I_2025"  # locations will be added in further down script
...
max(randomization$plotNo) # how many plots per location are there?

total_plots  <- length(randomization$plotNo)

total_plots 

### We dont have a layouts or maps yet, skip ###
...
trial <- randomization %>% 
  ...
  mutate(location = if_else(location == 1,"AL",
